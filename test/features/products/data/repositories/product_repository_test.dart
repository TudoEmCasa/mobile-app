...
void main() {
  ...
  test('createProduct persists and returns id with relational mapping', () async {
    final categoryId = await categoryRepository.createCategory('Fruits');
    final productTypeId = await productTypeRepository.createProductType('Apple', categoryId);
  ...
  test('watchProducts emits initial empty and then items', () async {
    ...
    await repository.createProduct(name: 'A', productTypeId: productTypeId, unitId: unitId, quantity: 1.0);
    await repository.createProduct(name: 'B', productTypeId: productTypeId, unitId: unitId, quantity: 2.0);
  ...
  test('updateProduct persists changes', () async {
    ...
    final productTypeId = await productTypeRepository.createProductType('Apple', categoryId);
    final unitId = await unitRepository.createUnit('Unit', 'u');
    final id = await repository.createProduct(name: 'A', productTypeId: productTypeId, unitId: unitId, quantity: 1.0);
  ...
  test('deleteProduct removes product and updates watchers', () async {
    ...
    final firstId = await repository.createProduct(name: 'A', productTypeId: productTypeId, unitId: unitId, quantity: 1.0);
    final secondId = await repository.createProduct(name: 'B', productTypeId: productTypeId, unitId: unitId, quantity: 2.0);
    ...
    final expectation = expectLater(
      ...
      emitsInOrder([
        ...
        predicate<List<ProductModel>>((list) => list.every((i) => i.id != firstId)),
      ]),
    );
